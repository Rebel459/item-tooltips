...
public class TooltipHelper {
    ...
    public static List<FormattedCharSequence> wrapTooltipLines(int screenWidth, int screenHeight, Font textRenderer, List<? extends Component> lines) {
        ...
        for (Component line : lines) {
            ...
            // Split allowing exact room for the prefix on the first line (and invisible indent on continuations)
            ...
            for (int i = 0; i < splitLines.size(); i++) {
                ...
                if (i == 0 || thisPrefixWidth == 0) {
                    // First line: keep original (visible prefix + styling intact)
                    // No prefix: no indent needed
                    wrapped.add(part);
                } else {
                    // Continuation lines: invisible pixel-perfect indent using custom font
